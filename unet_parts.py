...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        # 定义残差结构主路
        res_layers = [nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=False),#, stride=1
                      nn.BatchNorm2d(out_channels),
                      nn.ELU(inplace=True),#Leaky#ELU
                      nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, bias=False),#, stride=1
                      nn.BatchNorm2d(out_channels)]
        self.res_layers = nn.Sequential(*res_layers)
        # 定义残差结构旁路
        shortcut_layers = [
            nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0, bias=False),#, stride=1
            nn.BatchNorm2d(out_channels)]
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            #@@@@@原来self.conv = DoubleConv(in_channels, out_channels // 2, in_channels // 2)
            #self.conv = DoubleConv(in_channels, out_channels//2, in_channels // 2)
            self.conv = ResBlock(in_channels, out_channels//2)#是否//2
        else:
